...
function serviceLoader() {
  ...
  const closeGracefully = (signal) => {
    ...
    if (Object.keys(timeouts).length) logger.info('> Clear all timeouts');
  ...
  chain.db = (options) => {
    ...
    const pgDatabase = (options && options.database) ? options.pgDatabase : dbConfig.pgDatabase;
