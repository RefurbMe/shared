  ...
  return new Promise((resolve, reject) => {
    request(options, (err, res) => {
      ...
      if (res.statusCode >= 500) {
        logger.warn(`Request internal error [${res.statusCode}]: ${context} ${method} ${uri}: ${res.body.message || ''}`);
      }
      return resolve(res.body);
    });
  })
  .catch((e) => {
    logger.warn(`Request exception [${e.code}]: ${context} ${method} ${uri}: ${e.message}`);
    throw e;
  });
}
